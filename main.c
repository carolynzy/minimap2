...
<<<<<<< HEAD
	...
	{ "is-min-ins-len", ko_required_argument, 371 },
	{ "is-min-id",      ko_required_argument, 372 },
	{ "is-min-cov",     ko_required_argument, 373 },
	{ "is-flank-len",   ko_required_argument, 380 },
	{ "is-min-flank-id",ko_required_argument, 381 },
...
static int parse_del_report_mode(const char *s)
{
	...
	if (strcmp(s, "merged") == 0 || strcmp(s, "merge") == 0 || strcmp(s, "gap") == 0) return 1;
	fprintf(stderr, "[WARNING]\033[1;31m --del-report-mode only accepts 'separate' or 'merged'; using 'separate'.\033[0m\n");
	return 0;
}
...
static int read_is_seq_file(mm_mapopt_t *opt, const char *fn)
{
	...
	fp = mm_bseq_open(fn);
	if (fp == 0) return -1;
	s = mm_bseq_read(fp, INT64_MAX, 0, &n);
	mm_bseq_close(fp);
	if (n <= 0 || s == 0) return -1;
	old_n = opt->n_is_seq;
	opt->is_seq = (mm_is_seq_t*)realloc(opt->is_seq, (old_n + n) * sizeof(mm_is_seq_t));
	memset(&opt->is_seq[old_n], 0, n * sizeof(mm_is_seq_t));
	opt->n_is_seq += n;
	for (i = 0; i < n; ++i) {
		mm_is_seq_t *t = &opt->is_seq[old_n + i];
		t->name = s[i].name;
		t->len = s[i].l_seq;
		t->seq[0] = (uint8_t*)malloc(t->len);
		t->seq[1] = (uint8_t*)malloc(t->len);
		for (j = 0; j < t->len; ++j) {
			int c = seq_nt4_table[(uint8_t)s[i].seq[j]];
			t->seq[0][j] = c < 4? c : 4;
		}
		for (j = 0; j < t->len; ++j) {
			int c = t->seq[0][t->len - 1 - j];
			t->seq[1][j] = c < 4? 3 - c : 4;
		}
		free(s[i].seq);
		free(s[i].qual);
	...
	free(s);
	if (mm_verbose >= 3)
		fprintf(stderr, "[M::%s] loaded %d IS sequence(s) from %s\n", __func__, n, fn);
	return 0;
}

static int read_is_seq(mm_mapopt_t *opt, const char *fn_list)
{
	char *buf, *p, *q;
	int ret = 0;
	buf = strdup(fn_list);
	for (p = q = buf;; ++p) {
		if (*p == 0 || *p == ',' || *p == ';') {
			int c = *p;
			*p = 0;
			if (*q && read_is_seq_file(opt, q) != 0) ret = -1;
			if (c == 0 || ret != 0) break;
			q = p + 1;
		}
	}
...
static void free_is_seq(mm_mapopt_t *opt)
{
	int32_t i;
	for (i = 0; i < opt->n_is_seq; ++i) {
		free(opt->is_seq[i].name);
		free(opt->is_seq[i].seq[0]);
		free(opt->is_seq[i].seq[1]);
	}
	free(opt->is_seq);
	opt->is_seq = 0, opt->n_is_seq = 0;
}

static void append_path_value(char **dst, const char *val)
{
	size_t old_l = *dst? strlen(*dst) : 0, val_l = strlen(val);
	*dst = (char*)realloc(*dst, old_l + val_l + 2);
	if (old_l) (*dst)[old_l++] = ';';
	memcpy(*dst + old_l, val, val_l + 1);
}

static int json_value_after(const char *p, char *dst, int dst_l)
{
	int i = 0, quoted = 0;
	p = strchr(p, ':');
	if (p == 0) return 0;
...
static int read_is_config(mm_mapopt_t *opt, const char *fn, char **fn_is_seq, char **fn_is_ref_bed)
{
	FILE *fp;
	char buf[4096];
	fp = fopen(fn, "r");
	if (fp == 0) return -1;
	if (fgets(buf, sizeof(buf), fp) && (strchr(buf, '{') || strchr(buf, '['))) {
		long n;
		char *json, *p;
		fseek(fp, 0, SEEK_END);
		n = ftell(fp);
		fseek(fp, 0, SEEK_SET);
		json = (char*)calloc(n + 1, 1);
		if (json == 0) { fclose(fp); return -1; }
		if (fread(json, 1, n, fp) != (size_t)n) {
			free(json);
			fclose(fp);
			return -1;
		}
		for (p = json; (p = strstr(p, "\"fasta\"")) != 0; ++p)
			if (json_value_after(p, buf, sizeof(buf))) append_path_value(fn_is_seq, buf);
		for (p = json; (p = strstr(p, "\"is-seq\"")) != 0; ++p)
			if (json_value_after(p, buf, sizeof(buf))) append_path_value(fn_is_seq, buf);
		for (p = json; (p = strstr(p, "\"is_ref_bed\"")) != 0; ++p)
			if (json_value_after(p, buf, sizeof(buf))) { free(*fn_is_ref_bed); *fn_is_ref_bed = strdup(buf); }
		for (p = json; (p = strstr(p, "\"is-ref-bed\"")) != 0; ++p)
			if (json_value_after(p, buf, sizeof(buf))) { free(*fn_is_ref_bed); *fn_is_ref_bed = strdup(buf); }
#define JSON_NUM(KEY, FIELD, PARSER) do { p = strstr(json, "\"" KEY "\""); if (p && json_value_after(p, buf, sizeof(buf))) opt->FIELD = PARSER(buf); } while (0)
		JSON_NUM("is_window", is_window, mm_parse_num);
		JSON_NUM("is-window", is_window, mm_parse_num);
		JSON_NUM("is_min_ins_len", is_min_ins_len, mm_parse_num);
		JSON_NUM("is-min-ins-len", is_min_ins_len, mm_parse_num);
		JSON_NUM("is_min_id", is_min_id, atof);
		JSON_NUM("is-min-id", is_min_id, atof);
		JSON_NUM("is_min_cov", is_min_cov, atof);
		JSON_NUM("is-min-cov", is_min_cov, atof);
		JSON_NUM("is_flank_len", is_flank_len, mm_parse_num);
		JSON_NUM("is-flank-len", is_flank_len, mm_parse_num);
		JSON_NUM("is_flank_max_snp", is_flank_max_snp, atoi);
		JSON_NUM("is-flank-max-snp", is_flank_max_snp, atoi);
		JSON_NUM("is_min_flank_id", is_min_flank_id, atof);
		JSON_NUM("is-min-flank-id", is_min_flank_id, atof);
		JSON_NUM("del_report_mode", del_report_mode, parse_del_report_mode);
		JSON_NUM("del-report-mode", del_report_mode, parse_del_report_mode);
#undef JSON_NUM
		free(json);
		fclose(fp);
		return 0;
	...
	while (fgets(buf, sizeof(buf), fp)) {
		...
		*val++ = 0;
		...
		for (p = val; *p && *p != '\n' && *p != '\r'; ++p) {}
		*p = 0;
		if (strncmp(key, "--", 2) == 0) key += 2;
		if (strcmp(key, "is-seq") == 0 || strcmp(key, "fasta") == 0) append_path_value(fn_is_seq, val);
		else if (strcmp(key, "is-ref-bed") == 0) *fn_is_ref_bed = strdup(val);
		else if (strcmp(key, "is-window") == 0) opt->is_window = mm_parse_num(val);
		else if (strcmp(key, "is-min-ins-len") == 0) opt->is_min_ins_len = mm_parse_num(val);
		else if (strcmp(key, "is-min-id") == 0) opt->is_min_id = atof(val);
		else if (strcmp(key, "is-min-cov") == 0) opt->is_min_cov = atof(val);
		else if (strcmp(key, "is-flank-len") == 0) opt->is_flank_len = mm_parse_num(val);
		else if (strcmp(key, "is-flank-max-snp") == 0) opt->is_flank_max_snp = atoi(val);
		else if (strcmp(key, "is-min-flank-id") == 0) opt->is_min_flank_id = atof(val);
		else if (strcmp(key, "del-report-mode") == 0) opt->del_report_mode = parse_del_report_mode(val);
	}
	fclose(fp);
	return 0;
}
...
<<<<<<< HEAD
		...
		else if (c == 371) opt.is_min_ins_len = mm_parse_num(o.arg); // --is-min-ins-len
		else if (c == 372) opt.is_min_id = atof(o.arg); // --is-min-id
		else if (c == 373) opt.is_min_cov = atof(o.arg); // --is-min-cov
		else if (c == 380) opt.is_flank_len = mm_parse_num(o.arg); // --is-flank-len
		else if (c == 381) opt.is_min_flank_id = atof(o.arg); // --is-min-flank-id
		else if (c == 382) { // --is-config
			if (read_is_config(&opt, o.arg, &fn_is_seq, &fn_is_ref_bed) != 0) {
				fprintf(stderr, "[ERROR] failed to load IS config file '%s'\n", o.arg);
				return 1;
			}
		}
...
<<<<<<< HEAD
		...
		fprintf(fp_help, "    --is-window NUM        IS rescue neighborhood on each side in reference and query [%d]\n", opt.is_window);
		fprintf(fp_help, "    --is-min-ins-len NUM   minimum query segment length to test as IS [%d]\n", opt.is_min_ins_len);
		fprintf(fp_help, "    --is-min-id FLOAT      minimum query-to-IS identity [%g]\n", opt.is_min_id);
		fprintf(fp_help, "    --is-min-cov FLOAT     minimum query-to-IS coverage [%g]\n", opt.is_min_cov);
		fprintf(fp_help, "    --is-flank-len NUM     minimum flank anchor seed length [%d]\n", opt.is_flank_len);
...
<<<<<<< HEAD
		if (fn_is_ref_bed) {
			...
			else if (opt.n_is_seq == 0 && mm_idx_is_ref_fill_seqs(mi, &opt) != 0 && mm_verbose >= 2)
				fprintf(stderr, "[WARNING] failed to extract IS sequences from reference IS BED intervals\n");
		} else if (fn_is_seq) {
